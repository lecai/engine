...
namespace PPE
{
	...
	void CConnectorEx::Release()
	{
		...
		m_pConnector=NULL;
		PPE_DELETE_T( this, CConnectorEx, MEMCATEGORY_GENERAL);
	}
																...
																uint32 uPingTick)
	{
		...
		if(NULL != pOnPingServer)
		{
			...
			std::cout << "CConnectorEx RegisterTick " << uPingTick << std::endl;
			GetTickMgr()->RegisterTick(uPingTick, this);
		}
	}
	...
	{
		uint32 uCurTime = CTimer::GetSingleton().getRealTime();
		m_uPingValue = (uCurTime - m_uLastPingTime)/2; 
	}

	void		CConnectorEx::OnTick()
	{
		m_uLastPingTime = CTimer::GetSingleton().getRealTime();
